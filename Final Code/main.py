...
async def run():
	...
	coordinate_thresh = 20 # How many pixels we allow before we tell the drone to move
	...
	while(await drone.get_offboard_state() == True and vision.is_detecting):
		...
		if(vision.object_is_detected() and detectedObj.check_detected_status() == False):
			...
			detectedObj.set_config(area, box_dim, coordinate_thresh, area_thresh, vision.get_object_coordinates())
			print("\n-- Object Detected\n")
			...
			print("0:",coordinates[0], "1:",coordinates[1])
			if coordinates[0] > detectedObj.get_intial_coords()[0] + detectedObj.coordinate_thresh:
				await drone.left(move_x, 1.0)
				detectedObj.set_intial_coords([coordinates[0], coordinates[1]])
			elif coordinates[0] < detectedObj.get_intial_coords()[0] - detectedObj.coordinate_thresh:
				await drone.right(move_x, 1.0)
				detectedObj.set_intial_coords([coordinates[0], coordinates[1]])
			if coordinates[1] > detectedObj.get_intial_coords()[1] + detectedObj.coordinate_thresh:
				await drone.down(move_y, 1.0)
				detectedObj.set_intial_coords([coordinates[0], coordinates[1]])
			elif coordinates[1] < detectedObj.get_intial_coords()[1] - detectedObj.coordinate_thresh:
